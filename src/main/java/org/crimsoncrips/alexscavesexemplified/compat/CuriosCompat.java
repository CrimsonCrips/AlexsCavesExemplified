...
public class CuriosCompat {
    public static boolean hasLight(LivingEntity target){
        return target.isHolding(Ingredient.of(ACEItemTagGenerator.LIGHT)) || curiosLight(target);
    }
    ...
    private static boolean curiosLight(LivingEntity target){
        if (ModList.get().isLoaded("curiouslanterns")) {
            try{
                ...
                return handler.getStacksHandler("belt").orElseThrow().getStacks().getStackInSlot(0).is(ACEItemTagGenerator.LIGHT);
