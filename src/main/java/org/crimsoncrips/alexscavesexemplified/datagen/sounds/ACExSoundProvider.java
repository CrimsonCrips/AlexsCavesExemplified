...
public abstract class ACExSoundProvider extends SoundDefinitionsProvider {

	protected ACExSoundProvider(PackOutput output, ExistingFileHelper helper) {
		super(output, AlexsCavesExemplified.MODID, helper);
	}
	...
	public void generateNewSound(RegistryObject<SoundEvent> event, String baseSoundDirectory, int numberOfSounds, boolean subtitle) {
		...
		for (int i = 1; i <= numberOfSounds; i++) {
			definition.with(SoundDefinition.Sound.sound(new ResourceLocation(AlexsCavesExemplified.MODID, baseSoundDirectory + (numberOfSounds > 1 ? i : "")), SoundDefinition.SoundType.SOUND));
		}
	...
	public void makeMusicDisc(RegistryObject<SoundEvent> event, String discName) {
		this.add(event, SoundDefinition.definition()
				.with(SoundDefinition.Sound.sound(new ResourceLocation(AlexsCavesExemplified.MODID, "music/" + discName), SoundDefinition.SoundType.SOUND)
						.stream()));
	}
